...
class FileServices {
  static chooseimage(id) async {
    FilePickerResult? result = await FilePicker.platform.pickFiles(
        type: FileType.custom, allowedExtensions: ["png", "jpeg", "gif"]);
    if (result != null) {
      Uint8List? fileBytes = result.files.first.bytes;
      String fileName = result.files.first.name;
      uploadStorage(fileName, fileBytes, id);
    } else {
      print("error");
    }
  }

  static uploadStorage(fileName, fileBytes, id) async {
